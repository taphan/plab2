...
class Motob():
    ...
    def operationalize(self, motor_rec):
        ...
        if motor_rec[1] == None:
            duration = None # Kjør i evighet
        else:
            duration = motor_rec[1] / 60 # Snur

        if motor_rec[0] == 'L':
            self.motors.set_value([.5, -.5], duration) # Endrer på disse verdiene etter testing
        elif motor_rec[0] == 'R':
            self.motors.set_value([-.5, .5], duration) # 90 grader = 1 second, 180 grader = 3 seconds
        elif motor_rec[0] == 'F':
            self.motors.forward(.4,None)
        elif motor_rec[0] == 'S':
            self.motors.stop()
